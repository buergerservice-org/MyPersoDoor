...
int main(int argc, char** argv)
{
    ...
    hashkeyfile = fopen("hashkey.txt", "a+");
    PINfile = fopen("PIN.txt", "w+");

    std::cout << "\nbuergerservice.org e.V. workflowClient\n\n" << std::endl;
    ...
    std::cout << "Moechten Sie die PIN abspeichern? (Datei PIN.txt)\n   Achtung: Vorhandene PIN wird ueberschrieben! Das funktioniert nur fuer eine Person (zB Hausherr)\n   Bitte j oder n eingeben\n" ;
    std::string sinput;
    std::cin >> sinput;
    if(sinput=="j" || sinput=="J")
    {
        std::cout << "Die PIN soll abgespeichert werden. " << std::endl;
        fprintf(PINfile, PINstring.c_str());
    }
    std::cout << "Moechten Sie den erzeugten hashkey abspeichern? (Datei hashkey.txt)\n   Achtung: Der hashkey wird angehaengt.\n   Bitte j oder n eingeben\n" ;
    sinput="";
    std::cin >> sinput;
    if(sinput=="j" || sinput=="J")
    {
        std::cout << "Der hashkey soll abgespeichert werden. " << std::endl;
        //fprintf(PINfile, PINstring.c_str());
    }
    ...
    if(sinput=="j" || sinput=="J")
    {
        ...
        //std::cout << "Der hashkey soll abgespeichert werden. " << std::endl;
        fprintf(hashkeyfile, outputstring.c_str());
    }
